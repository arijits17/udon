	...
	//displayGlobalVars();
        if(load) {
        	for(Module::iterator funcItr = module.begin(); funcItr != module.end(); ++funcItr) {
				...
				errs() << "[DEBUG] running on function: " << funcName << '\n';
...
void DaikonPass::doInit(Module *module) {
	...
	hookFunctionBegin = cast<Function>(module->getOrInsertFunction("clap_hookFuncBegin",
        				FunctionType::getVoidTy(module->getContext())/*ret type*/,
        				Type::getInt8PtrTy(module->getContext())/*char* funcName*/, NULL)); //might need to add param type of char *
	
	hookForInt = cast<Function>(module->getOrInsertFunction("hookForInt", 
					FunctionType::getVoidTy(module->getContext())/*ret type*/,
					Type::getInt8PtrTy(module->getContext())/*char* varName*/,
					Type::getInt64Ty(module->getContext())/*int64_t data*/, NULL)); // might need to add params of type char * for variable name, and int for value	
	
	hookForPoint = cast<Function>(module->getOrInsertFunction("hookForPoint",
					FunctionType::getVoidTy(module->getContext())/*ret type*/,
					Type::getInt8PtrTy(module->getContext())/*char* varName*/,
					Type::getInt64PtrTy(module->getContext())/* void* */, NULL));

	endOfHookBegin = cast<Function>(module->getOrInsertFunction("endOfHookBegin",
					FunctionType::getVoidTy(module->getContext())/*ret type*/, NULL));

	hookFunctionEnd = cast<Function>(module->getOrInsertFunction("clap_hookFuncEnd",
					FunctionType::getVoidTy(module->getContext())/*ret type*/,
        				Type::getInt8PtrTy(module->getContext())/*char* funcName*/, NULL));
	endOfHookEnd = cast<Function>(module->getOrInsertFunction("endOfHookEnd", 
					FunctionType::getVoidTy(module->getContext()), NULL));
}
...
void DaikonPass::hookAtFunctionStart(Function *func) {
	...
	//insert clap_hookFuncBeing
	//clap_hookFuncBegin now only handles printing thread info and function name, no variables
	...
	/// acquire target location to insert clap_hookFuncBegin
	Instruction *target = NULL;
		...
		if(StoreInst *globSt = dyn_cast<StoreInst>(&*instItr)) {
			...
			// if the pointer operand is == the clap dummy variable
			if(ptrOp && ptrOp == clapDummyVar) {
				++instItr;
				// increment instruction iterator and check to see if it is a call instruction
				if(CallInst *callInst = dyn_cast<CallInst>(&*instItr)) {
					if(callInst->getCalledFunction()->getName().trim().equals("clap_store_post")) {
						// if the call instuction is of name clap_store_post increment the iterator
						++instItr;
						if(instItr != inst_end(func)) { // make sure that the iterator isnt at the end of the function
							target = &*instItr;
							break;
						}
					}
	...
	if(target == NULL)
		assert(0 && "target is NULL");

	//INSERT hookFuncBegin BEFORE THE TARGET
	...
	// Convert the global string (funcNameValue) which is a pointer to an
	// array of int8s to the type of the first paramter of the
	// hookFunctionCall. The first parameter has is a pointer to an int8.
	BitCastInst *bc_inst = new BitCastInst(funcNameValue, hookFunctionBegin->getFunctionType()->getParamType(0), "gcast", target);
	std::vector<Value*> funcNameVect;
	funcNameVect.push_back(bc_inst);

	//insert the hookFuncBegin
	assert(hookFunctionBegin->getFunctionType()->getParamType(0) == bc_inst->getType());
	Instruction *hookFunctionBeginInstruction = CallInst::Create(hookFunctionBegin,funcNameVect);
	...
	// Handle the globals first
	insertGlobals(target, module);	

	
	//now handle the function parameters
	insertArguments(target, module, func);	

	//insert endOfHookBegin
	Instruction *hookEndOfHookBegin = CallInst::Create(endOfHookBegin);
	hookEndOfHookBegin->insertBefore(target);
...
void DaikonPass::hookAtFunctionEnd(Function *func) {
	...
	// array of int8s to the type of the first paramter of the
	// hookFunctionCall. The first parameter has is a pointer to an int8.
	BitCastInst *bc_inst = new BitCastInst(funcNameValue, hookFunctionEnd->getFunctionType()->getParamType(0), "gcast", target);
	std::vector<Value*> funcNameVect;
	funcNameVect.push_back(bc_inst);

	//insert the hookFuncBegin
	assert(hookFunctionEnd->getFunctionType()->getParamType(0) == bc_inst->getType());
	Instruction *hookFunctionEndInstruction = CallInst::Create(hookFunctionEnd,funcNameVect);
	...
	if(hasValidReturn) {
	      ...
	      BitCastInst* valNameCast = new BitCastInst(valName, hookForInt->getFunctionType()->getParamType(0), "nameCast", target);
	      if(val->getType()->isIntegerTy()) // if the argument is an integer
			{
				insertHookInt( val, target, valNameCast);
			}
			else if(val->getType()->isPointerTy() ) // if the argument is a pointer
			{
				insertHookPoint(val, target, valNameCast);							
			}
			else
			{
			       ; //not handled
	...
	Instruction *endOfHookEndInstruct = CallInst::Create(endOfHookEnd);
	endOfHookEndInstruct->insertBefore(target);
}
...
void DaikonPass::dumpDeclFileAtEntryAndExit(Function *func,string EntryOrExit, fstream &declFile) {
	...
	if(declFile.is_open()) {
        ...
        errs() << "[DEBUG] processed decl file\n";
}
...
void DaikonPass::insertHookGlobInt( Value* gVal, Instruction* target, BitCastInst* gValNameCast)
{
	// create the call instruction for hookforint( char* variableName, int value)
       // load the global pointer value
       LoadInst* gValLoad = new LoadInst(gVal , "GlobalLoad", target);
       
       //sign extend the loaded value to 64 bits
       Value *loadedCast = CastInst::CreateSExtOrBitCast( gValLoad, int64Type, "loadedTo64Int", target); 
       
       //build the argument vector
       std::vector<Value *> args;
       args.push_back(gValNameCast);
       args.push_back(loadedCast);

       //insert the hookforInt
       CallInst::Create(hookForInt, args  ,"", target);

}

void DaikonPass::insertHookInt(Value* val, Instruction* target, BitCastInst* valNameCast)
{
       //sign extend the value
       Value *signExtended = CastInst::CreateSExtOrBitCast( val, int64Type, "loadedTo64Int", target);

       std::vector<Value *> args;
       args.push_back(valNameCast);
       args.push_back(signExtended);
...
void DaikonPass::insertHookPoint( Value* val, Instruction* target, BitCastInst* valNameCast)
{
	Value *ptrCast = CastInst::CreatePointerCast(val, ptr64Type, "ptrTo64ptr", target);

	std::vector<Value *> args;
	args.push_back(valNameCast);
	args.push_back(ptrCast);

	CallInst::Create(hookForPoint, args, "", target);

}

void DaikonPass::insertGlobals(Instruction* target, Module* module)
{
	for(vector<GlobalVariable*>::iterator globalItr = globalList.begin(); globalItr != globalList.end() ; ++globalItr) {
		Value *val = *globalItr;
		...
		BitCastInst* gValNameCast = new BitCastInst(valName, hookForInt->getFunctionType()->getParamType(0), "gcast", target);


		// gval- value, valName- variable name, type - varible type (only int for now)
		...
		{
		      if(gVal->getType()->getElementType()->isIntegerTy()) // if the global pointer points to an int
		      {
	        		insertHookGlobInt(gVal, target, gValNameCast);
		      }
		      else
		      {
			      ; //not handled yet
...
void DaikonPass::insertArguments( Instruction* target, Module* module, Function* func)
{
	for(Function::arg_iterator argItr = func->arg_begin(); argItr != func->arg_end(); ++argItr) {
		Argument *arg = &*argItr;
		...
		//cast the name balue to the function param type
		BitCastInst* valNameCast = new BitCastInst(valName, hookForInt->getFunctionType()->getParamType(0), "nameCast", target);
