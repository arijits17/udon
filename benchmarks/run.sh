...
# Check if the shell has all the proper exports
if [ -z ${CONTROLLER:-} ]
then
  cd ../Udon || die "Error: Udon directory not found"
  source exports.sh || die "Error: error sourceing exports"
  cd -
fi
...
$CONTROLLER rtool clean
# rebiuild
#controller_ui.py rtool comp
$CONTROLLER rtool comp
...
#($TIME  $CONTROLLER rtool run 0 2 1) 2> hapset.stderr.out 1>hapset.stdout.out
...
#done
#
## Count the number of invariants generated. 
...
#($TIME $CONTROLLER rtool run 0 1 2) 2> $PCB_STDERR 1>$PCB_STDOUT
#echo "Time Return: $?"
## last line of stderr is the output of time
...
then
  ...
  rm -f $TIME_OUTPUT
  date
  ($TIME $CONTROLLER rtool comp) 2>comp.stderr.out
  extract_time_and_add `tail -n 1 comp.stderr.out`
  # keep track of the time to run the test
  ($TIME $CONTROLLER rtool run 0 0 0) 2> $DPOR_STDERR 1>$DPOR_STDOUT
  echo "Time Return: $?"
  # last line of stderr is the output of time
...
#date
#($TIME $CONTROLLER daikon rep $DAIK_REPS) 2>kvas.stderr.out 1>kvas.stdout.out
#echo "Time Return: $?"
## double check that any old time file is not around
#rm -f $TIME_OUTPUT
